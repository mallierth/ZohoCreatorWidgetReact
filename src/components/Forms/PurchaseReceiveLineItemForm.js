...
const PurchaseReceiveLineItemForm = ({
  ...
  useEffect(() => {
    console.log("PurchaseReceiveLineItemForm.js data", data);
    console.log("PurchaseReceiveLineItemForm.js loadData", loadData);
  }, [data, loadData]);

  useEffect(() => {
    console.log("PurchaseReceiveLineItemForm.js state change", state);
    ...
    if (onChange) {
      ...
      onChange({ ...state.currentData, ...temp, Selected: false });
    }
  }, [state]);
  ...
  return (
    ...
    >
      ...
      {loadData.map((lineItem) => (
        <Grid key={lineItem.ID} container rowSpacing={0} columnSpacing={2}>
...
PurchaseReceiveLineItemForm.propTypes = {
...
PurchaseReceiveLineItemForm.defaultProps = {
  loadData: {},
  massUpdating: false,
  massUpdateRecordIds: [],
};
...
const Wrapper = (props) => {
  return (
    <React.Suspense fallback={<ContextCircularProgressLoader />}>
      <PurchaseReceiveLineItemForm {...props} />
    </React.Suspense>
  );
};
