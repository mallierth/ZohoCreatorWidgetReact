...
const CustomerRoomForm = ({
	...
	useEffect(() => {
		console.log(`CustomerRoomForm.js state change`, state);
					...
					<ThemeCard
						...
						<GridFormSectionWrapper>
							...
							<GridInputWrapper
								massUpdating={massUpdating}
								hidden={
									massUpdating && !massUpdateFieldList.includes('Drawings_Name')
								}>
								<TextField
									label='Drawings Name'
									value={state.currentData.Drawings_Name}
									onChange={(e) => mountData('Drawings_Name', e.target.value)}
								/>
							</GridInputWrapper>
							<GridInputWrapper
								massUpdating={massUpdating}
