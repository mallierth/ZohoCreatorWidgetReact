...
const PurchaseOrderLineItemForm = ({
						...
						<ThemeCard header='Pricing Details' sx={{ mt: { xs: 1, sm: 0 } }}>
							<Grid container spacing={2} direction='row'>
								...
								<Grid item xs={12} md={6}>
									<TextField
										label='Cost Subtotal'
										value={lineItemDataState.Cost_Subtotal}
										type='number'
										onChange={(e) =>
											lineItemDispatch({
												type: 'COST_SUBTOTAL',
								...
								<Grid item xs={12} md={6}>
									<TextField
										label='Cost Total'
										value={lineItemDataState.Cost_Total}
										type='number'
...
PurchaseOrderLineItemForm.propTypes = {
...
PurchaseOrderLineItemForm.defaultProps = {
	loadData: {},
	massUpdating: false,
	massUpdateRecordIds: [],
};
...
const Wrapper = (props) => {
	return (
		<React.Suspense fallback={<ContextCircularProgressLoader />}>
			<PurchaseOrderLineItemForm {...props} />
		</React.Suspense>
	);
};
