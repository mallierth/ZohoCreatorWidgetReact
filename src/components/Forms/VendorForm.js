...
const VendorForm = ({
	...
	useEffect(() => {
		console.log(`VendorForm.js state change`, state);
					...
					<ThemeCard
						...
						<GridFormSectionWrapper>
							...
							<GridInputWrapper
								...
								hidden={
									massUpdating &&
									!massUpdateFieldList.includes('Account_Number')
								}>
								<TextField
									label='Account Number'
									value={state.currentData.Account_Number}
									onChange={(e) => mountData('Account_Number', e.target.value)}
								/>
							</GridInputWrapper>
							<GridInputWrapper
								massUpdating={massUpdating}
								hidden={
									massUpdating && !massUpdateFieldList.includes('Tax_ID_Number')
								}>
								<TextField
									label='Tax ID Number'
									value={state.currentData.Tax_ID_Number}
									onChange={(e) => mountData('Tax_ID_Number', e.target.value)}
								/>
							</GridInputWrapper>
							<GridInputWrapper
								massUpdating={massUpdating}
								hidden={
									massUpdating &&
									!massUpdateFieldList.includes('Ordered_Through')
								}>
								<TextField
									select
									label='Ordered Through'
									value={state.currentData.Ordered_Through}
									required
									error={error.Ordered_Through}
									helperText={
										error.Ordered_Through
											? 'Please enter a value for this required field'
											: ''
									}
									onChange={(e) =>
										mountData('Ordered_Through', e.target.value)
									}>
									{columns
										.filter(
											(column) => column.valueKey === 'Ordered_Through'
										)[0]
										.options.map((option) => (
							...
							{/* Sections */}
							<GridInputWrapper>
								<ThemeCard header='Contact Info' elevation={8}>
									<Grid container spacing={2}>
										<Grid item xs={12}>
											<TextField
												...
												helperText={
													"This email address will be prioritized for Purchasing over the Primary Contact's Email"
												}
											/>
										</Grid>
										<Grid item xs={12}>
											<TextField
												label='Email Cc'
												value={state.currentData.Email_Cc}
												onChange={(e) => mountData('Email_Cc', e.target.value)}
											/>
										</Grid>
										<Grid item xs={12}>
											<TextField
												label='Phone'
												value={state.currentData.Phone}
												onChange={(e) => mountData('Phone', e.target.value)}
											/>
										</Grid>
										<Grid item xs={12}>
											<TextField
							...
							<GridInputWrapper>
								<ThemeCard header='Address' elevation={8}>
									<Grid container spacing={2}>
										<Grid item xs={12}>
											<TextField
												label='Street'
						...
						<TabbedSectionContent>
							{tabValue === 'Notes' ? (
								...
								<CustomTable
									...
									defaultCriteria={`Void_field=false && Vendor==${id ? id : 0}`}
									tabTable
								/>
							) : null}
						</TabbedSectionContent>
