...
const DemoForm = ({
	...
	useEffect(() => {
		console.log(`DemoForm.js state change`, state);
				...
				{massUpdating && massUpdateFieldList.length === 0 ? (
					...
					<ThemeCard
						...
						<GridFormSectionWrapper>
							...
							<GridInputWrapper>
								<ThemeCard header='Product Information' elevation={8}>
									<Grid container spacing={2}>
										<Grid item xs={12}>
											<LookupField2
												...
												defaultValue={state.currentData.Price_Book_Items}
												onChange={(e) => mountData('Price_Book_Items', e)}
												helperText={
													error.Price_Book_Items
														? 'Please enter a value for this required field'
														: null
												}
												required
												error={error.Price_Book_Items}
												multiSelect
												endAdornment={
