...
const EstimateForm = ({
	...
	//https://creatorapp.zoho.com/visionpointllc/av-professional-services/#Search1?Type=Estimate&ID=3860683000011594075
	...
	useEffect(() => {
		console.log(`EstimateForm.js state change`, state);
		...
		if (
			...
			const thisAlert = {
				...
				action: null,
				message:
					'No Tax: Make sure taxes will be applied at the time of invoice or account for them on this estimate',
			};
		...
		if (
			...
			const thisAlert = {
				...
				action: null,
				message:
					'This Estimate has been voided, so be aware that your changes will not trigger workflows and this record will be excluded from database calculations',
			};
					...
					<ThemeCard
						...
						<GridFormSectionWrapper>
							...
							<GridInputWrapper
								massUpdating={massUpdating}
								hidden={
									massUpdating && !massUpdateFieldList.includes('Order_Number')
								}>
								<TextField
									label='Order Number'
									value={state.currentData.Order_Number}
									onChange={(e) => mountData('Order_Number', e.target.value)}
								/>
							</GridInputWrapper>
							<GridInputWrapper
								massUpdating={massUpdating}
							...
							<GridInputWrapper
								...
								<TextField
									...
									value={state.currentData.Customer_Notes}
									multiline
									onChange={(e) => mountData('Customer_Notes', e.target.value)}
								/>
							</GridInputWrapper>
							<GridInputWrapper
								massUpdating={massUpdating}
								...
								<TextField
									...
									value={state.currentData.Terms_Conditions}
									multiline
									onChange={(e) =>
										mountData('Terms_Conditions', e.target.value)
									}
								/>
							</GridInputWrapper>
						</GridFormSectionWrapper>
						...
						<TabbedSectionContent>
							{tabValue === 'Line Items' ? (
								<CustomTable
									...
									defaultCriteria={`(Estimate==${id} && Deleted=false)`}
									enableShiftControls
			...
			<RenderPopup
				...
				<PrintRecord
					...
					defaultShowLineItemDetails={
						state.currentData.Type === 'Estimate' ||
